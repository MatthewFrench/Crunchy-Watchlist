;(() => {
  ...
  try {
    ...
    corePrimitives = corePrimitivesModule.createCorePrimitives({
    ...
    apiContracts = apiContractsModule.createApiContracts({
      windowRef: window,
      navigatorRef: window.navigator,
      runtimeEvent,
      parseDateMs: (value) => parseDateMs(value),
      getWatchlistSeriesId: (entry) => getWatchlistSeriesId(entry),
      getWatchHistorySeriesId: (entry) => getWatchHistorySeriesId(entry),
    ...
    const bootstrapHelpersRuntime = runtimeBootstrapHelpersModule.createBootstrapHelpersRuntime({
      ...
      normalizeAudioLocale: (value) => normalizeAudioLocale(value),
  ...
  async function storageGet(key, fallback) {
    return storageAdapter.get(key, fallback)
  }
  ...
  function exposeDebugApi() {
    window.__CW_WATCHLIST_CURATOR_DEBUG__ = {
      ...
      dumpSeriesApiData: (query) => dumpSeriesApiData(query),
      printSeriesApiData: (query) => printSeriesApiData(query),
    }
  }

  try {
    const runtimeLifecycle = runtimeLifecycleModule.createRouteLifecycle({
      state,
      runtimeEvent,
      isWatchlistPath,
    ...
    if (!runtimeLifecycle || typeof runtimeLifecycle.processWatchlist !== 'function') {
      throw new Error('[CW] Missing processWatchlist lifecycle handler')
    }
    if (typeof runtimeLifecycle.startRouteWatcher !== 'function') {
      throw new Error('[CW] Missing startRouteWatcher lifecycle handler')
    }
    if (typeof runtimeLifecycle.syncRoute !== 'function') {
      throw new Error('[CW] Missing syncRoute lifecycle handler')
    }

    processWatchlist = () => runtimeLifecycle.processWatchlist()
    startRouteWatcher = () => runtimeLifecycle.startRouteWatcher()
    syncRoute = () => runtimeLifecycle.syncRoute()
  } catch (_) {
    // no-op
  }

  try {
    const stateLoader = runtimeStateLoaderModule.createStateLoader({
      state,
      storageGet,
      runtimeEvent,
      normalizeStoredWatchHistoryCache,
    ...
    if (!stateLoader || typeof stateLoader.loadInitialState !== 'function') {
      throw new Error('[CW] Missing state loader')
    }
