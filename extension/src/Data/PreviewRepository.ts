type TokenEntry = {
  accessToken?: string;
};
type BoundaryValue = LooseRecord[string];
type BoundaryRecord = Record<string, BoundaryValue>;
type PreviewRecord = BoundaryRecord;
type PreviewPayloadRoot = PreviewRecord | null;

type PreviewEntry = {
  seriesId: string;
  streamsLink: string;
  panelId: string;
  canonicalEpisodeKey: string;
};

type RefreshBearerTokenFn = () => Promise<string>;

type PreviewRequestOptions = {
  label: string;
  bearerToken?: string;
  refreshBearerToken?: RefreshBearerTokenFn;
};

type PreviewState = {
  previewCache: Record<string, string | null>;
  previewInflight: Map<string, Promise<string | null>>;
};

type PreviewContext = {
  state: PreviewState;
  resolveApiHref: (value: string | null | undefined) => string;
  getAccessToken: (forceRefresh?: boolean) => Promise<TokenEntry | null>;
  fetchWithResilience: (url: string, requestInit: RequestInit, options: PreviewRequestOptions) => Promise<Response>;
  createAuthRefreshHandler: (tokenEntry: TokenEntry | null) => RefreshBearerTokenFn | undefined;
  pushApiTrace: (endpoint: string, payload: PreviewRecord) => void;
  runtimeEvent: (event: string, payload?: PreviewRecord) => void;
};

type PreviewOptions = {
  state?: BoundaryValue;
  resolveApiHref?: BoundaryValue;
  getAccessToken?: BoundaryValue;
  fetchWithResilience?: BoundaryValue;
  createAuthRefreshHandler?: BoundaryValue;
  pushApiTrace?: BoundaryValue;
  runtimeEvent?: BoundaryValue;
};

function requireFunction<T extends (...args: never[]) => BoundaryValue>(name: string, value: BoundaryValue): T {
  if (typeof value !== 'function') {
    throw new Error(`[CW] Missing preview dependency: ${name}`);
  }
  return value as T;
}

function toRecord(value: BoundaryValue): PreviewRecord {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return {};
  }

  return value as PreviewRecord;
}

function toTokenEntry(value: BoundaryValue): TokenEntry | null {
  if (!value || typeof value !== 'object') {
    return null;
  }

  return value as TokenEntry;
}

function toPreviewEntry(value: BoundaryValue): PreviewEntry {
  const record = toRecord(value);
  return {
    seriesId: typeof record.seriesId === 'string' ? record.seriesId : '',
    streamsLink: typeof record.streamsLink === 'string' ? record.streamsLink : '',
    panelId: typeof record.panelId === 'string' ? record.panelId : '',
    canonicalEpisodeKey: typeof record.canonicalEpisodeKey === 'string' ? record.canonicalEpisodeKey : '',
  };
}

function createPreviewContext(options: PreviewOptions = {}): PreviewContext {
  const state = options.state && typeof options.state === 'object' ? (options.state as PreviewState) : null;
  if (!state) {
    throw new Error('[CW] Missing preview state');
  }

  if (!state.previewCache || typeof state.previewCache !== 'object') {
    state.previewCache = {};
  }
  if (!(state.previewInflight instanceof Map)) {
    state.previewInflight = new Map();
  }

  return {
    state,
    resolveApiHref: requireFunction('resolveApiHref', options.resolveApiHref),
    getAccessToken: requireFunction('getAccessToken', options.getAccessToken),
    fetchWithResilience: requireFunction('fetchWithResilience', options.fetchWithResilience),
    createAuthRefreshHandler: requireFunction('createAuthRefreshHandler', options.createAuthRefreshHandler),
    pushApiTrace:
      typeof options.pushApiTrace === 'function' ? (options.pushApiTrace as PreviewContext['pushApiTrace']) : () => {},
    runtimeEvent:
      typeof options.runtimeEvent === 'function' ? (options.runtimeEvent as PreviewContext['runtimeEvent']) : () => {},
  };
}

function findFirstMediaUrlInternal(
  context: PreviewContext,
  value: BoundaryValue,
  visited: Set<object> = new Set(),
): string | null {
  if (typeof value === 'string') {
    const text = value.trim();
    if (!text) {
      return null;
    }

    if (!/^https?:\/\//i.test(text) && !text.startsWith('/')) {
      return null;
    }

    const looksLikeMedia =
      /\.(m3u8|mp4|webm|m4v|mpd|jpg|jpeg|png|webp|avif)(\?|$)/i.test(text) ||
      /(?:playlist|manifest|stream|preview|video|thumbnail|poster|image)/i.test(text);

    if (!looksLikeMedia) {
      return null;
    }

    return context.resolveApiHref(text) || text;
  }

  if (!value || typeof value !== 'object') {
    return null;
  }

  if (visited.has(value)) {
    return null;
  }
  visited.add(value);

  if (Array.isArray(value)) {
    for (const item of value) {
      const found = findFirstMediaUrlInternal(context, item, visited);
      if (found) {
        return found;
      }
    }
    return null;
  }

  const record = value as PreviewRecord;
  for (const key of Object.keys(record)) {
    const found = findFirstMediaUrlInternal(context, record[key], visited);
    if (found) {
      return found;
    }
  }

  return null;
}

function parsePreviewUrlFromPayloadInternal(context: PreviewContext, payload: PreviewRecord | null): string | null {
  const payloadRecord = toRecord(payload);
  const previewRecord = toRecord(payloadRecord.preview);
  const streamsRecord = toRecord(payloadRecord.streams);
  const adaptiveHlsRecord = toRecord(streamsRecord.adaptive_hls);
  const hlsRecord = toRecord(streamsRecord.hls);

  const directCandidates: Array<string | null | undefined> = [
    typeof payloadRecord.preview_url === 'string' ? payloadRecord.preview_url : undefined,
    typeof payloadRecord.previewUrl === 'string' ? payloadRecord.previewUrl : undefined,
    typeof payloadRecord.preview_image === 'string' ? payloadRecord.preview_image : undefined,
    typeof payloadRecord.previewImage === 'string' ? payloadRecord.previewImage : undefined,
    typeof previewRecord.url === 'string' ? previewRecord.url : undefined,
    typeof previewRecord.image === 'string' ? previewRecord.image : undefined,
    typeof payloadRecord.url === 'string' ? payloadRecord.url : undefined,
    typeof adaptiveHlsRecord.url === 'string' ? adaptiveHlsRecord.url : undefined,
    typeof adaptiveHlsRecord[''] === 'string' ? adaptiveHlsRecord[''] : undefined,
    typeof hlsRecord.url === 'string' ? hlsRecord.url : undefined,
    typeof hlsRecord[''] === 'string' ? hlsRecord[''] : undefined,
  ];

  for (const candidate of directCandidates) {
    const resolved = context.resolveApiHref(candidate);
    if (resolved) {
      return resolved;
    }
  }

  const nestedCandidates = [adaptiveHlsRecord, hlsRecord, streamsRecord];

  for (const candidate of nestedCandidates) {
    const found = findFirstMediaUrlInternal(context, candidate);
    if (found) {
      return found;
    }
  }

  return null;
}

function getPreviewCacheKeyInternal(context: PreviewContext, entry: PreviewEntry): string {
  const streamsUrl = context.resolveApiHref(entry.streamsLink);
  if (streamsUrl) {
    return `streams:${streamsUrl}`;
  }

  const panelId = entry.panelId.trim();
  if (panelId) {
    return `episode:${panelId}`;
  }

  const canonicalEpisodeKey = entry.canonicalEpisodeKey.trim();
  if (canonicalEpisodeKey) {
    return `canonical:${canonicalEpisodeKey}`;
  }

  const seriesId = entry.seriesId.trim();
  if (seriesId) {
    return `series:${seriesId}`;
  }

  return '';
}

function createPreviewRequestOptionsInternal(
  context: PreviewContext,
  tokenEntry: TokenEntry | null,
): PreviewRequestOptions {
  const requestOptions: PreviewRequestOptions = {
    label: 'preview request',
  };
  const refreshBearerToken = context.createAuthRefreshHandler(tokenEntry);
  if (typeof refreshBearerToken === 'function') {
    requestOptions.refreshBearerToken = refreshBearerToken;
  }

  if (typeof tokenEntry?.accessToken === 'string') {
    requestOptions.bearerToken = tokenEntry.accessToken;
  }

  return requestOptions;
}

async function parsePreviewPayloadFromResponseInternal(
  context: PreviewContext,
  response: Response,
  streamsUrl: string,
  seriesId: string,
  previewCacheKey: string,
): Promise<PreviewPayloadRoot> {
  let payload: BoundaryValue = null;
  try {
    payload = await response.json();
  } catch (_) {
    context.runtimeEvent('preview-contract-warning', {
      reason: 'invalid-json-payload',
      seriesId,
      cacheKey: previewCacheKey,
    });
  }

  context.pushApiTrace('preview', {
    at: Date.now(),
    request: {
      url: streamsUrl,
      seriesId,
      cacheKey: previewCacheKey,
    },
    response: payload,
  });

  return normalizePreviewPayloadRootInternal(context, payload, seriesId, previewCacheKey);
}

function normalizePreviewPayloadRootInternal(
  context: PreviewContext,
  payload: BoundaryValue,
  seriesId: string,
  previewCacheKey: string,
): PreviewPayloadRoot {
  if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
    context.runtimeEvent('preview-contract-warning', {
      reason: 'invalid-payload-root',
      seriesId,
      cacheKey: previewCacheKey,
    });
    return null;
  }

  return payload as PreviewRecord;
}

async function fetchPreviewFromStreamsInternal(
  context: PreviewContext,
  streamsUrl: string,
  seriesId: string,
  previewCacheKey: string,
): Promise<string | null> {
  const tokenEntry = toTokenEntry(await context.getAccessToken(false));
  const requestOptions = createPreviewRequestOptionsInternal(context, tokenEntry);
  const response = await context.fetchWithResilience(
    streamsUrl,
    {
      credentials: 'include',
    },
    requestOptions,
  );

  if (!response.ok) {
    return null;
  }

  const payload = await parsePreviewPayloadFromResponseInternal(
    context,
    response,
    streamsUrl,
    seriesId,
    previewCacheKey,
  );
  return parsePreviewUrlFromPayloadInternal(context, payload);
}

async function fetchPreviewUrlForEntryInternal(
  context: PreviewContext,
  inputEntry: BoundaryValue,
): Promise<string | null> {
  const entry = toPreviewEntry(inputEntry);
  const seriesId = entry.seriesId;
  if (!seriesId) {
    return null;
  }

  const previewCacheKey = getPreviewCacheKeyInternal(context, entry);
  if (!previewCacheKey) {
    return null;
  }

  if (Object.hasOwn(context.state.previewCache, previewCacheKey)) {
    return context.state.previewCache[previewCacheKey] || null;
  }

  const inflightEntry = context.state.previewInflight.get(previewCacheKey);
  if (inflightEntry) {
    return inflightEntry;
  }

  const streamsUrl = context.resolveApiHref(entry.streamsLink);
  if (!streamsUrl) {
    context.state.previewCache[previewCacheKey] = null;
    return null;
  }

  const inflight = fetchPreviewFromStreamsInternal(context, streamsUrl, seriesId, previewCacheKey)
    .catch(() => null)
    .then((previewUrl) => {
      context.state.previewCache[previewCacheKey] = previewUrl || null;
      return previewUrl || null;
    })
    .finally(() => {
      context.state.previewInflight.delete(previewCacheKey);
    });

  context.state.previewInflight.set(previewCacheKey, inflight);
  return inflight;
}

function createPreviewRepository(options: PreviewOptions = {}) {
  const context = createPreviewContext(options);
  return {
    fetchPreviewUrlForEntry: (entry: BoundaryValue) => fetchPreviewUrlForEntryInternal(context, entry),
  };
}

const previewRepositoryRuntime = {
  createPreviewRepository,
};

export function createPreviewRepositoryRuntime(): object {
  return previewRepositoryRuntime;
}
