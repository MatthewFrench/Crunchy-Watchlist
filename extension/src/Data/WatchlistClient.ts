...
type WatchlistPayloadDataEnvelope = {
  rows: WatchlistRow[];
  total: number | null;
};
type WatchlistTracePayload = BoundaryRecord;
type RefreshBearerTokenFn = () => Promise<string>;

type WatchlistRequestOptions = {
  label: string;
  bearerToken?: string;
...
type WatchlistContext = {
  ...
  createAuthRefreshHandler: (tokenEntry: TokenEntry | undefined) => RefreshBearerTokenFn | undefined;
  resolveApiHref: (pathWithQuery: string) => string;
  parsePayloadDataEnvelope: (endpoint: string, payload: BoundaryValue) => WatchlistPayloadDataEnvelope;
  ...
  pushApiTrace: (endpoint: string, record: WatchlistTracePayload) => void;
  runtimeEvent: (event: string, payload?: WatchlistTracePayload) => void;
...
type WatchlistClientOptions = {
  ...
  auditWatchlistRowsContract?: BoundaryValue;
  ...
  watchlistPageSize?: BoundaryValue;
  watchlistMaxPages?: BoundaryValue;
  watchlistParallelRequests?: BoundaryValue;
};
...
function createWatchlistContext(options: WatchlistClientOptions = {}): WatchlistContext {
  return {
    ...
    parsePayloadDataEnvelope: requireFunction(
      'parsePayloadDataEnvelope',
      options.parsePayloadDataEnvelope,
    ) as WatchlistContext['parsePayloadDataEnvelope'],
...
): WatchlistRequestOptions {
  const requestOptions: WatchlistRequestOptions = {
...
async function fetchWatchlistPageInternal(
  ...
  const payloadEnvelope = context.parsePayloadDataEnvelope('watchlist', payload);
  const rows = payloadEnvelope.rows;
  context.auditWatchlistRowsContract(rows);
  const total = payloadEnvelope.total ?? rows.length;
  if (payloadEnvelope.total == null) {
    context.runtimeEvent('watchlist-contract-warning', {
      reason: 'invalid-total-value',
      totalValue: toRecord(payload).total,
...
const watchlistClientRuntime = {
  createWatchlistClient,
};

export function createWatchlistClientRuntime(): object {
  return watchlistClientRuntime;
}
