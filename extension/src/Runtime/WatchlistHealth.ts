...
type RuntimeSettings = Record<string, BoundaryValue>;
...
type RuntimeState = {
  ...
  curatedInflight: Promise<BoundaryValue> | null;
  curatedPendingRequests: BoundaryList;
...
type WatchlistHealthContext = {
  ...
  processWatchlist: () => Promise<BoundaryValue>;
...
type WatchlistHealthOptions = {
  ...
  processWatchlist?: BoundaryValue;
  syncRouteRuntime?: BoundaryValue;
  blankShellReloadStorageKey?: BoundaryValue;
  blankShellReloadCountStorageKey?: BoundaryValue;
  blankShellReloadCooldownMs?: BoundaryValue;
  blankShellReloadMaxPerSession?: BoundaryValue;
  blankShellRecoveryStabilizeMs?: BoundaryValue;
  blankShellCheckIntervalMs?: BoundaryValue;
  identityChurnWarnCooldownMs?: BoundaryValue;
  identityChurnMinCreatedDelta?: BoundaryValue;
  identityChurnWarnRate?: BoundaryValue;
};
...
function normalizeRateNumber(value: BoundaryValue, fallback: number): number {
  const number = Number(value);
...
function toRuntimeState(value: BoundaryValue): RuntimeState | null {
...
function resolveWindowRef(value: BoundaryValue): Window {
  if (value && typeof value === 'object') {
...
function isHiddenElement(value: BoundaryValue): boolean {
  const element = value as {
    style?: {
      display?: string;
    };
...
function getWatchlistHealthIssueInternal(context: WatchlistHealthContext): string {
  ...
  const loadingUiVisible = loadingUiConnected && !(typeof loadingUiDisplay === 'string' && loadingUiDisplay === 'none');
...
export function createWatchlistHealthRuntime(options: WatchlistHealthOptions = {}): WatchlistHealthRuntime {
