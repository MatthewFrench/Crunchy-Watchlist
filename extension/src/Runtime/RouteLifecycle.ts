(() => {
  ...
  type RouteWatcherState = {
    ...
    historyOriginalPushState: ((this: History, ...args: unknown[]) => unknown) | null;
    historyOriginalReplaceState: ((this: History, ...args: unknown[]) => unknown) | null;
  ...
  function isMutationWithinOwnedHost(target: unknown, host: Element | null): boolean {
    ...
    ['pushState', 'replaceState'].forEach((methodName) => {
      ...
      try {
        ...
        if (methodName === 'pushState') {
          routeWatcherState.historyOriginalPushState = original as (this: History, ...args: unknown[]) => unknown;
        } else if (methodName === 'replaceState') {
          routeWatcherState.historyOriginalReplaceState = original as (this: History, ...args: unknown[]) => unknown;
        }
      } catch (_error) {
