...
const preloadModuleUrl = pathToFileURL(
  path.join(process.cwd(), 'extension', 'src', 'data', 'history-repository-preload.ts'),
).href
...
describe('history-repository-preload', () => {
  beforeEach(async () => {
  ...
  it('marks history as unavailable when token data is missing', async () => {
    ...
    const fetchWithResilience = vi.fn(async () => new Response(JSON.stringify({ data: [], total: 0 }), { status: 200 }))
