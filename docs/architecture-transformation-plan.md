...
- `extension/Content.css`: 576 lines and `extension/ContentResponsive.css`: 52 lines (split style surfaces; both under strict target).
- `extension/src/Domain/CorePrimitives.ts`: 539 lines (below strict runtime target `<= 600`).
...
- Largest remaining runtime functions:
  ...
  - `createCuratedInterfaceControlsInternal`: 63 lines (`extension/src/Ui/ControlsView.ts`)
...
- Hotspot reductions completed in this cycle:
  ...
  - `extension/Content.js`: 892 -> 794 lines
  - `extension/Content.js`: 794 -> 597 lines
  - `extension/src/Data/HistoryRepositoryPreload.ts`: 759 -> 699 lines
  - `extension/src/Data/HistoryRepositoryPreload.ts`: 699 -> 514 lines
  - `extension/src/Runtime/ContentComposition.ts`: 709 -> 568 lines
  - `extension/Content.css`: 628 -> 576 lines
...
- Current largest Playwright spec is `tests/RankingAndProgress.spec.ts` at 438 lines.
...
- Tooling/stack constraints from full codebase review:
  ...
  - architecture metrics now scan all extension CSS files (not only `Content.css`) so style-surface splits remain visible in budget reporting.
...
Latest `npm run arch:metrics` shows architecture budgets are currently compliant at both file and function levels:
...
```text
extension/
  Content.js                     # transitional composition-root runtime entry
  Content.css
  ContentResponsive.css
  manifest.json
  src/
    Runtime/
      ContentBootstrap.ts
      ContentComposition.ts
      ContentRuntimeSetup.ts
      ContentRuntimeSetupComposition.ts
      ContentRuntimeSetupDataInitialization.ts
      BootstrapConfig.ts
  ...
  Types/
    BrowserGlobals.d.ts
    CommonRuntimeTypes.d.ts
    ContentCompositionTypes.d.ts
    HistoryRepositoryPreloadTypes.d.ts
  generated/
...
| M7 | Completed | TBD | Cleanup and standards lock restored: warning/refactor hotspots are `0`, key runtime owners are decomposed, and docs/workflows are synchronized with current architecture gates. |
...
3. Transitional composition root remains composition-only and stays within strict target budgets (`<= 600`) without direct business/domain logic regrowth.
...
- Complete for architecture-transformation scope. Remaining items are optimization and release-confidence cadence, not core architecture debt.

## 10) Progress Update History

Latest pass additions (2026-02-27):

1. Completed strict-size decompositions for near-threshold owners by extracting `CuratedPanelGridTransitions` and `RatingsRepositoryCacheSupport`, reducing:
   - `CuratedPanelGrid.ts`: `629 -> 399`
   - `RatingsRepository.ts`: `599 -> 492`
2. Kept prior seam ownership extractions in place (`CuratedRenderableMergeSupport`, `InterfaceShellHostLifecycle`, `ContentRuntimeBootstrapSessionSupport`) and aligned runtime delegation boundaries.
3. Added focused seam-level unit coverage for `CuratedPanelGridTransitions` and `RatingsRepositoryCacheSupport`, and updated existing runtime/data tests to load extracted owners in manifest-parity order.
4. Split CI verification lanes into fast default checks and a dedicated high-cost `release-confidence` workflow (manual + nightly).
5. Re-ran architecture gates and metrics with full green status (`test:unit`, `typecheck`, `lint`, `format:check`, `lint:firefox`, `build:webext`, `build:runtime:safari:checked`, `arch:metrics`).
...
192. Re-verified full architecture gates after episode-primitives extraction:
   ...
   - `npm run build:safari`
   - `npm run arch:metrics`
193. Added optional blended secondary sorting for curated ranking:
   - introduced `secondarySortMode` setting and control ownership.
   - implemented blended ranking in `Runtime/CuratedRenderable.ts` using averaged primary/secondary rank positions.
   - preserved deterministic ordering with primary/secondary tie-breaks.
194. Reduced architecture warning friction after blended-sort feature growth:
   - refactored `loadCuratedEntriesInternal` in `Runtime/CuratedLoader.ts` into focused helper owners.
   - refactored `loadInitialStateInternal` in `Runtime/StateLoader.ts` into settings/cache hydration helpers.
   - cleared warning-level function hotspots reported in architecture metrics.
195. Re-verified architecture baseline gates after this pass:
   - `npm run typecheck`
   - `npm run lint`
   - `npm run format`
   - `npm run format:check`
   ...
   - `npm run arch:metrics`
196. Added favorites-focused curated filtering support:
   - added `Genre: Favorites` as a first-class control option.
   - implemented favorites-only filtering in `Runtime/CuratedRenderable.ts` via a dedicated sentinel filter mode (`__favorites__`).
   - added Playwright + unit coverage for favorites filtering behavior and filtered stats parity.
197. Added loading-state request diagnostics and progress counters:
   - `Runtime/CuratedLoader.ts` now tracks duplicate-preserving pending requests plus started/completed counters.
   - `Runtime/CuratedPanel.ts` now renders request-progress details (`Completed X of Y • In progress Z`) and full in-flight request lists in loading empty states.
   - `Runtime/RuntimeStore.ts` and refresh-reset flows were updated to persist/reset request-progress counters deterministically.
198. Completed status-line simplification in curated cards:
   - removed the legacy dedicated "Next unwatched" row from card body rendering.
   - merged next-episode values into the primary status line (`Up Next: Sx Ey`, `Continue: Sx Ey`) in `Ui/CuratedCardView.ts`.
   - added unit coverage to prevent regressions in merged status text behavior.
199. Expanded coverage for new runtime and UX behavior:
   - updated `tests/UiBehavior.spec.ts` loading assertions for request progress diagnostics.
   - updated `tests/RankingAndProgress.spec.ts` to validate merged status text and favorites filtering behavior.
   - added/updated unit suites for controls, renderable filtering, panel loading diagnostics, runtime loader progress tracking, runtime store defaults, interface-shell refresh reset, state loader defaults, and curated-card view status rendering.
200. Re-verified full architecture gates after runtime/UI feature and diagnostics updates:
   - `npm ci`
   - `npm run typecheck`
   - `npm run lint`
   - `npm run format`
...
205. Re-verified full architecture gate suite after latest refactors and Xcode updates:
   ...
   - `npm run build:safari`
   - `npm run arch:metrics`
206. Extracted bootstrap prelude ownership from `Content.js`:
   - added `extension/src/Runtime/ContentBootstrap.ts` for diagnostics runtime resolution, route-gate resolution, and bootstrap runtime module validation.
   - updated `extension/manifest.json` script ordering to load `src/Runtime/ContentBootstrap.js` before `Content.js`.
   - added `tests/Unit/Runtime/ContentBootstrap.test.ts` for gate-denied and bootstrap-success prelude coverage.
207. Closed the near-term bootstrap and history preload line-budget goals:
   - reduced `extension/Content.js` from `899` source lines (`900` metrics accounting) to `793` source lines (`794` metrics accounting).
   - reduced `extension/src/Data/HistoryRepositoryPreload.ts` from `759` lines to `698` source lines (`699` metrics accounting) by inlining helper resolvers and reducing indirection.
208. Re-verified the full architecture and release-confidence gates after these reductions:
   - `npm ci`
   - `npm run typecheck`
   - `npm run lint`
   - `npm run format:check`
   - `npm run test:unit` (145 passed)
   - `npm run pw:live:smoke`
   - `npm run lint:firefox`
   - `npm run test:e2e` (81 passed)
   - `npm run build:webext`
   - `npm run build:safari`
   - `npm run arch:metrics`
209. Updated architecture standards and plan priorities for the next cycle:
   - shifted priority from the completed `<= 800`/`<= 700` targets to long-term strict budget work (`Content.js <= 600`).
   - called out dependency-hygiene debt (`npm ci` currently reports 3 moderate vulnerabilities in transitive dev-tooling packages).
210. Completed strict-size reduction for remaining runtime composition/decomposition surfaces:
   - `extension/Content.js`: `794` -> `597`.
   - `extension/src/Runtime/ContentComposition.ts`: `709` -> `568`.
   - `extension/src/Data/HistoryRepositoryPreload.ts`: `699` -> `514`.
211. Extracted large type contracts into dedicated ambient runtime typing modules:
...
212. Removed the last strict-size style outlier by splitting responsive/animation styling:
   - `extension/Content.css`: `628` -> `576`
   - added `extension/ContentResponsive.css` and updated `extension/manifest.json` CSS ordering.
213. Hardened architecture observability after style split:
   - updated `scripts/architecture-metrics.mts` to track all `extension/**/*.css` files instead of a single hardcoded stylesheet.
214. Re-verified full architecture and release-confidence gates after strict-budget completion:
   - `npm ci`
   - `npm run typecheck`
   - `npm run lint`
   - `npm run format:check`
   ...
   - `npm run build:safari`
   - `npm run arch:metrics`
215. Closed dependency-hygiene debt without regressing Firefox lint/build parity:
   - added `package.json` override for `addons-linter@^9.9.1`.
   - validated `npm ci` + `npm audit` return clean (`0` vulnerabilities).
   - re-validated `npm run lint:firefox` with upgraded linter dependency chain.
216. Tightened architecture-regrowth guardrails for near-threshold owners:
   - updated `scripts/architecture-metrics.mts` overrides so `NativeBridge.ts`, `AuthClient.ts`, `ContentComposition.ts`, `HistoryRepositoryCache.ts`, and `Content.js` now warn at `> 600` and refactor at `> 700`.
   - preserved baseline global budgets for non-overridden modules.
217. Hardened Safari build determinism in local and CI composition roots:
   - updated `scripts/build-safari-macos.sh` to use explicit arch-aware destination selection (`platform=macOS,arch=<arm64|x86_64>`).
   - updated `.github/workflows/build-extensions.yml` Safari job to run `npm run build:safari` so generated runtime/typecheck gates run before artifact packaging.
218. Re-verified full architecture and release-confidence gates after dependency + CI/runtime hardening:
   - `npm ci`
   - `npm run typecheck`
   - `npm run lint`
   - `npm run format:check`
...
237. Completed remaining function hotspot decomposition and re-validated full release-confidence gates:
   - reduced all warning-level runtime functions under the 70-line warning budget (`createBootstrapFinalizeRuntime`, `createCuratedCardActionsInternal`, `createCuratedLoaderContext`, `mergeRenderableEntryInternal`, `createCuratedCardThumbInternal`, `startRuntime`).
   - re-ran `format:check`, `lint`, `typecheck`, `test:unit` (`204`), `lint:firefox`, `test:e2e` (`102`), `build:webext`, `build:safari`, and `arch:metrics` with green results.
238. Extracted setup data-initialization ownership and added seam coverage:
   - moved setup-time trace/contracts/auth/storage/repository initialization from `ContentRuntimeSetup.ts` into `ContentRuntimeSetupDataInitialization.ts`.
   - rewired setup orchestration to consume `createContentRuntimeSetupDataInitializationRuntime`.
   - added new runtime unit suites: `ContentRuntimeSetupDataInitialization.test.ts`, `ContentRuntimeSetup.test.ts`, and `ContentRuntimeBootstrapSession.test.ts`.
   - this cycle validation: `format:check`, `lint`, `typecheck`, `test:unit` (`209`), `lint:firefox`, `build:webext`, `build:runtime:safari:checked`, and `arch:metrics`.
   - deferred by request to release-candidate batch: `test:e2e`, full signed `build:safari`.
239. Completed optimization-priority item 1 by extracting bootstrap finalize/session flow ownership:
   - added `extension/src/Runtime/ContentRuntimeBootstrapFinalizeFlow.ts` and delegated finalize flow orchestration from `ContentRuntimeBootstrapSession.ts`.
   - reduced `ContentRuntimeBootstrapSession.ts` from `789` to `636` lines.
240. Completed optimization-priority item 3 with additional runtime-owner decomposition:
   - added `extension/src/Runtime/CuratedRenderableListProcessing.ts` and delegated filter/sort/list processing from `CuratedRenderable.ts`.
   - added `extension/src/Runtime/CuratedInteractionsControls.ts` and delegated controls binding from `CuratedInteractions.ts`.
   - added `extension/src/Runtime/CuratedLoaderPendingRequests.ts` and `extension/src/Runtime/CuratedLoaderLoadCycle.ts`, delegating pending diagnostics and load-cycle/failure ownership from `CuratedLoader.ts`.
   - reduced `CuratedLoader.ts` from `742` to `484` lines while keeping function budgets compliant.
241. Updated runtime wiring and tests for manifest-parity owner loading:
   - updated `extension/manifest.json` content-script order to load all new runtime owners before dependents.
   - updated unit runtime loaders in `tests/Unit/Runtime/CuratedLoader.test.ts`, `tests/Unit/Runtime/CuratedRenderable.test.ts`, `tests/Unit/Runtime/CuratedInteractions.test.ts`, and `tests/Unit/Runtime/ContentRuntimeBootstrapSession.test.ts`.
242. Re-verified architecture gates after this decomposition slice:
   - `npm run format:check`
   - `npm run lint`
   - `npm run typecheck`
...
6. Architecture metrics remain the source-of-truth for status and should drive optimization priorities.
...
1. **Architecture transformation: 100% complete for the current defined scope**.
2. **Tooling and release-confidence hardening: 99% complete for this cycle scope**.

Completion breakdown:

1. Runtime ownership extraction: `100%`
   - Runtime responsibilities are segmented by owner modules with composition-root orchestration boundaries.
2. TypeScript/tooling modernization: `100%`
   - TypeScript, Biome lint/format, unit/E2E gates, and build pipelines are active and green.
3. Architecture budget compliance: `100%`
   - No warning-level file or function hotspots remain in latest `npm run arch:metrics`.
4. Verification confidence coverage: `99%`
   - Current cycle gates passed:
     - `format:check`, `lint`, `typecheck`, `test:unit`, `lint:firefox`, `build:webext`, `build:runtime:safari:checked`, `arch:metrics`.
   - Deferred by request for release-candidate batch:
     - `test:e2e`
     - signed `build:safari`
5. Dependency/toolchain hygiene: `100%` for current scope
   - Install/build/test parity is green; ongoing upgrades remain operational maintenance work.

Gap-to-100% (what is still lacking):

1. One verification gap remains in this cycle: deferred high-cost release-confidence checks (`test:e2e`, signed/notarized `build:safari`).
2. Remaining architecture work is optimization/headroom, not baseline transformation debt.

Current unresolved blockers and high-friction items:

1. No hard blockers currently prevent execution.
2. High-friction: high-cost release gates (`test:e2e`, signed/notarized `build:safari`) are time/infra intensive, so they should remain batched at release-confidence points.
3. High-friction: aggregate `npm run test:e2e` can intermittently hit cross-project Playwright setup timeouts even when individual project suites pass (`chromium`/`firefox`/`webkit`); mitigation is to run projects sequentially in the release-confidence lane or lower aggregate concurrency.
4. Non-blocking Xcode friction remains: AppIntents metadata notice (`No AppIntents.framework dependency found.`).
