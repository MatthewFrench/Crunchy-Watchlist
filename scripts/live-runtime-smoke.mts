...
function assertArrayEqual(label: string, left: string[], right: string[]): void {
  ...
  for (let index = 0; index < left.length; index += 1) {
    if (left[index] !== right[index]) {
      throw new Error(`Mismatch for ${label} at index ${index}: expected "${left[index]}", received "${right[index]}".`)
    }
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    process.stdout.write(`[live-smoke] Building generated runtime at ${runtimeDir}\n`)
    await runCommand('tsx', [buildScriptPath, '--out', runtimeDir])
    ...
    process.stdout.write(
      `[live-smoke] OK: manifest parity + asset existence validated (${runtimeAssets.cssFiles.length} css, ${runtimeAssets.jsFiles.length} js).\n`,
    )
  } finally {
    if (!keepOutput) {
