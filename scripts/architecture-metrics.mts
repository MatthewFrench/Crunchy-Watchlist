...
type FunctionClassification = 'ok' | 'warning' | 'refactor';

export type ArchitectureMetricsSnapshot = {
  fileRows: FileRow[];
  runtimeFunctionSpans: FunctionSpan[];
...
export const ARCH_FUNCTION_WARN_THRESHOLD = FILE_SIZE_BUDGETS.functionWarn;
export const ARCH_FUNCTION_REFACTOR_THRESHOLD = FILE_SIZE_BUDGETS.functionRefactor;
...
export function classifyFunctionSpan(length: number): FunctionClassification {
  if (length > FILE_SIZE_BUDGETS.functionRefactor) {
    return 'refactor';
  }
  if (length > FILE_SIZE_BUDGETS.functionWarn) {
    return 'warning';
  }
  return 'ok';
}
...
function renderMarkdown(args: {
  ...
  const functionTableRows = args.topFunctions
    .map((fn) => {
      const classification = classifyFunctionSpan(fn.length);
      const status = classification === 'refactor' ? 'Refactor' : classification === 'warning' ? 'Warning' : 'OK';
...
export async function collectArchitectureMetrics(): Promise<ArchitectureMetricsSnapshot> {
  ...
  for (const fn of functionSpans) {
    if (classifyFunctionSpan(fn.length) === 'refactor') {
  ...
  const warningFunctions = runtimeFunctionSpans
    .filter((fn) => classifyFunctionSpan(fn.length) === 'warning')
