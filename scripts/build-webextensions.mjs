...
function parseRequestedBrowser(argv) {
  ...
  if (!supportedBrowsers.includes(value)) {
...
function finalizeManifest(baseManifest, browser) {
...
async function buildBrowserPackage(baseManifest, browser) {
...
async function run() {
  const requestedBrowser = parseRequestedBrowser(process.argv.slice(2));
  ...
  for (const iconPath of Object.values(extensionIcons())) {
    await ensureFileExists(path.join(extensionDir, iconPath));
  }

  const results = [];
  for (const browser of browsers) {
    const result = await buildBrowserPackage(baseManifest, browser);
    results.push(result);
  }
