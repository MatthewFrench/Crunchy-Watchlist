...
async function runHotReload(page: Page): Promise<void> {
  if (!hotReloadEnabled || hotReloadInFlight) {
    return;
  }
...
const fileWatchers = createFileWatchers(page);

console.log(
  `Live WebKit session started (generated runtime injection from ${runtimeOutputDir} on every watchlist navigation).`,
);
